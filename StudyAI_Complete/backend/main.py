...
client = OpenAI(api_key=API_KEY, timeout=45.0, max_retries=0) if API_KEY else None
...
def _responses_create(**kwargs):
    ...
    try:
        ...
        # Keep server logs useful while returning a stable error to the app.
        msg = str(exc)
        low = msg.lower()
        if "401" in low or "invalid_api_key" in low or "authentication" in low:
            raise RuntimeError("OPENAI_AUTH_ERROR") from exc
        if "429" in low or "rate_limit" in low or "quota" in low:
            raise RuntimeError("OPENAI_RATE_LIMIT") from exc
        if "timeout" in low or "timed out" in low:
            raise RuntimeError("OPENAI_TIMEOUT") from exc
        raise RuntimeError("OPENAI_REQUEST_ERROR") from exc
...
@app.get("/ai/test")
def ai_test_get():
    return {"ok": True, "endpoint": "/ai/test", "method": "POST", "model": MODEL, "ai_configured": bool(API_KEY)}
...
def ai_test(body: AITestRequest, request: Request):
    ...
    if not API_KEY:
        raise HTTPException(503, "مفتاح OpenAI غير مضبوط على Render.")
    try:
        ...
        if code == "OPENAI_AUTH_ERROR":
            raise HTTPException(502,"مفتاح OpenAI غير صالح أو منتهي على Render.")
        if code == "OPENAI_RATE_LIMIT":
            raise HTTPException(429,"وصلنا إلى حد OpenAI/الرصيد المؤقت. لا تعيد الضغط عدة مرات؛ جرّب لاحقاً.")
        if code == "OPENAI_TIMEOUT":
            raise HTTPException(504,"OpenAI لم يرد خلال المهلة. جرّب مرة واحدة بعد قليل.")
        raise HTTPException(502,"تعذر الوصول إلى OpenAI حالياً. راجع سجل Render لمعرفة السبب.")
...
def feature(body: FeatureRequest, request: Request):
    ...
    try:
        ...
        msg=str(exc).lower()
        if "429" in code or "rate_limit" in code or code == "OPENAI_RATE_LIMIT":
            raise HTTPException(429,"تم الوصول إلى حد AI مؤقتاً. انتظر ثم أعد المحاولة.")
...
def chat(body: ChatRequest, request: Request):
    ...
    try:
        ...
        code=str(exc)
        if code == "OPENAI_AUTH_ERROR": raise HTTPException(502,"مفتاح OpenAI غير صالح أو منتهي على Render.")
        if code == "OPENAI_RATE_LIMIT": raise HTTPException(429,"وصلنا إلى حد OpenAI/الرصيد المؤقت. لا تعيد الطلب عدة مرات؛ جرّب لاحقاً.")
        if code == "OPENAI_TIMEOUT": raise HTTPException(504,"OpenAI لم يرد خلال المهلة. جرّب مرة واحدة بعد قليل.")
        raise HTTPException(502,"تعذر الوصول إلى OpenAI حالياً. راجع سجل Render لمعرفة السبب.")
